...
- Fixed the local pre-tag check being narrower than CI's. `pytest` now
  ...
  cut before this held: the first failed a gate needing `tomllib` on a 3.10
  runner, the second failed lint the local suite never ran, and the third
  failed the conformance slice because the first attempt at this guard
  extracted one command from a step that runs two. Verified to fire on both
  halves before being trusted.
...
- Fixed `imports: [malleus]` failing to resolve without an explicit `--map`. The installed root is now the last-resort fallback, after any local or vendored copy, so root-currency drift stays detectable. Before this, `pip install malleus-dev` followed by a correct schema and a first run of the inspector produced a construction heresy: the tool manufactured a finding against correct work at precisely the moment an adopter decides whether it is worth using. One field report had twelve correct schemas reported broken this way.
- Fixed fourteen of LinkML's nineteen built-in ranges being construction failures. `uri`, `double`, `decimal`, `date`, `time`, `curie`, `uriorcurie`, `ncname`, `objectidentifier`, `nodeidentifier`, `jsonpointer`, `jsonpath`, `sparqlpath` and `date_or_datetime` are accepted and each validates as its base kind: `double` and `decimal` as numbers, the rest as strings. Refusing a legal type punished adopters for using their schema language correctly, and it cost two independent projects a schema in the same week, one on `uri` and one on `double`. The lexical form is not enforced, which is new pending capability `lexical-format-validation`, stated rather than implied and pinned by a test.
- Fixed a construction failure hiding its blast radius. Rite one failing short-circuits the run, so a single unresolvable range silently blinded every later rite on that file and on every file importing it: one field inspection found seven of eight rites unexecuted across an entire repository with nothing in the report saying so. The report now names the rites that did not run, and excludes rites the rubric disabled, since those were switched off rather than skipped.
...
- Added the installed malleus version and the resolved root path to the inspector's header, so one command answers "which malleus am I actually running against". A stale install previously announced itself as a confusing `ImportError` from the bootstrap probe; one adopter was running 0.1.0 with a 0.6.0 checkout beside it.
- Added `ROADMAP.md` and `design/COMMITMENT_LIFECYCLE.md`. The roadmap records six lessons sent up by an adopting project plus one from our own use, each with the evidence that earned it and two standing promotion rules. The design note works through a lifecycle facade that is deliberately not being built yet, since level 5 has no consumer outside this repository and an API designed from one example encodes that example's accidents permanently.
