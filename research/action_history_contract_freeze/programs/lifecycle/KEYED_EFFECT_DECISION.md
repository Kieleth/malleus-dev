...
Status: PROPOSED, operator decision required. Research OPTIONAL_PROFILE only.
No instruction, executor, production path or public API changes in this slice.
...
The current `SET_PROTOCOL_STATE` instruction declares a target, name and value.
...
`execution.json` retains the exact missing operand as a candidate `keys` list.
The current closed instruction schema rejects it with `INSTRUCTION_SHAPE`.
...
## Recommended refinement, not implemented
...
The decision requested is whether to adopt this explicit keyed assignment in
the research instruction definition. It does not authorize runtime work.
