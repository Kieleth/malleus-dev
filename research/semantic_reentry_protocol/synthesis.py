...
from research.semantic_reentry_protocol.consumer import (
    Reason, ReentryRefusal, Status, assess_request, canonical, digest,
)
...
def synthesize(*, contract, context, view_bytes, plan_bytes, source_bytes,
               mapping_bytes, partial_contract, contract_view, base_state,
               history_profile):
    """Return one existing KCS, no candidate, or typed refusal. Never admit."""
    assessment = assess_request(
        contract=contract, view_bytes=view_bytes, plan_bytes=plan_bytes,
        source_bytes=source_bytes, mapping_bytes=mapping_bytes,
    ...
    for value, expected_type in (
        ...
        if type(value) is not expected_type:
            raise ReentryRefusal(Reason.MALFORMED_INPUT,
                                 f"expected immutable {expected_type.__name__}")
    view, plan = json.loads(view_bytes), json.loads(plan_bytes)
    coordinates = {field: getattr(context, attribute)
                   for field, attribute in _COORDINATES.items()}
    retained = {member.record_id: {"sha256": member.identity, "role": member.role}
                for member in context.retained_inputs}
    if (coordinates != view["coordinates"] or retained != view["retained"]
            or partial_contract.identity != context.contract_identity):
        raise ReentryRefusal(Reason.STALE_BASE,
                             "read input and Core composition context differ")
    if plan["history_profile"] != {
        "profile_id": history_profile.profile_id, "sha256": history_profile.identity
    }:
        raise ReentryRefusal(Reason.INPUT_IDENTITY_MISMATCH,
    ...
    compiled = api.compile_population_plan(
        plan, partial_contract=partial_contract, contract_view=contract_view,
        base_state=base_state, history_profile=history_profile,
    )
    if (compiled.canonical_plan_bytes != plan_bytes
            or compiled.status is not api.PopulationPlanStatus.CHANGE_SET
            or len(compiled.operations) != 1):
        raise ReentryRefusal(Reason.PRESERVATION_VIOLATION,
    ...
    if (operation.operation_type != "CREATE_ENTITY"
            ...
            or operation.source_id is not None or operation.target_id is not None
    ...
    candidate = api.compose_change_set(
        context=context, change_set_id=f"change:{compiled.plan_id}",
