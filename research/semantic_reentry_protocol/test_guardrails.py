...
def test_valid_but_wrong_row_citation_cannot_justify_e7(shop):
    ...
    edit_plan(
        request,
        lambda plan: plan["derivations"][2].update({"locator": "row:0:quantity"}),
    )
...
def test_missing_binding_cannot_hide_a_fabricated_property(shop):
    ...
    mapping["state_bindings"] = [
        member
        for member in mapping["state_bindings"]
    ...
    history.append_anchors(
        anchors=(artifact(record_id, mapping_bytes),),
    ...
    request["contract"]["request"].update(
        {"mapping_id": record_id, "mapping_sha256": digest(mapping_bytes)}
    )
...
def test_rebound_projection_cannot_fabricate_a_satisfied_accepted_state(shop):
    ...
    view["record_history"][E7] = {
        "supersedes_record_id": E4,
        "superseded_by": None,
