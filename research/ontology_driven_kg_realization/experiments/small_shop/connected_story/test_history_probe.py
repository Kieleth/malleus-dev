...
def test_occurrences_and_replacement_survive_public_reopen(tmp_path):
    ...
    assert {row["id"] for row in replay.graph.query("SupplierOrderOccurrence")} == {
        "e4",
        "e7",
    }
    assert len(replay.graph.query("SupplierOrder")) == 1
    assert [
        (row["id"], row["ordered_quantity"])
        for row in replay.graph.query("SupplierOrderState")
    ] == [("state:B:e7", 2)]
    assert replay.record_history["state:B:e4"].superseded_by == "state:B:e7"
    assert {
        row["entity_id"]
        for row in replay.graph.query_event_participations(event_id="e7")
    } == {"supplier-order:B"}
    ...
    for record_id in replay.record_history:
        ...
        assert (
            ...
            == (Path(__file__).parent / "sources/table-1.jsonl").read_bytes()
        )
