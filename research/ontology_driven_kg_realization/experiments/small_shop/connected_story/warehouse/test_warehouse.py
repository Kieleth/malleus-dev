...
def executed(subject, tmp_path_factory):
    ...
    refreshed = maintained.refresh(
        expected_head_hash=after.ledger_head,
        expected_event_count=after.ledger_event_count,
    )
    assert subject.read_warehouse(refreshed) == subject.read_warehouse(after)
...
def test_exact_figure_inventory_has_no_invented_actor(subject):
    ...
    assert all(
        set(row) == {"event_id", "activity", "time_text", "item_ids"} for row in rows
    )
    assert sum(map(len, rows)) == 52
    assert {x for row in rows for x in row["item_ids"]} == {
        "X1",
        "X2",
        "X3",
        "Y1",
    ...
    assert rows[-1] == {
        "event_id": "e32",
        "activity": "Retrieve",
        "time_text": "09-05 09:45",
        "item_ids": ["Y2"],
    }
    assert boundary["source"]["license"] == "CC-BY-4.0"
    assert boundary["source"]["figure_url"].endswith("/figures/14")
    original = list(
        map(json.loads, (run.HERE / "sources/table-1.jsonl").read_bytes().splitlines())
    )
    assert set(IDS).isdisjoint(row["event_id"] for row in original)
...
def test_ontology_adds_only_three_activity_values(subject):
    ...
    assert set(values) - set(before["enums"]["ShopActivity"]["permissible_values"]) == {
        "SCAN",
        "STORE",
        "RETRIEVE",
    }
...
def test_same_history_revision_preserves_the_complete_prefix(subject, executed):
    ...
    assert (
        run.digest(prefix)
        == json.loads((run.HERE / "run_receipt.json").read_bytes())["ledger_sha256"]
    )
    assert path.read_bytes().startswith(prefix)
    assert len(after.change_sets) == 34
    assert len(after.contract_revisions) == 1
    assert [change.kind for change in after.contract_revisions[0].changes] == [
        "ADD_ENUM_VALUE"
    ] * 3
    ...
    assert (
        after.graph.query("SupplierOrderState", product_code="Y")[0]["ordered_quantity"]
        == 2
    )
...
def test_unit_paths_gain_only_the_sourced_warehouse_steps(subject, executed):
    ...
    assert all(
        report["events"][identifier]["participants"].keys() == {"ITEM"}
...
def test_every_new_field_traces_to_exact_figure_rows(subject, executed):
    ...
    for ordinal, row in enumerate(rows):
        ...
        targets = [
            event_id,
            "participation:" + event_id + ":item:" + row["item_ids"][0],
        ]
        locators = set()
        for record_id in targets:
            ...
            assert (
                trace.change_set.contract_identity == replay.partial_contract.identity
            )
        ...
        assert locators == {
            f"row:{ordinal}:{field}"
            for field in ("event_id", "activity", "time_text", "item_ids[0]")
        }
    assert (
...
def test_recorded_receipt_is_reproducible(subject, executed):
    ...
    assert subject.receipt(replay, path.read_bytes(), prefix) == json.loads(
        (subject.HERE / "receipt.json").read_bytes()
    )


def test_public_schema_command_and_read_only_cli(subject, executed):
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert json.loads(result.stdout) == subject.read_warehouse(replay)
    assert path.read_bytes() == saved
