    ...
    program["steps"][:0] = [
        {
            ...
            "refusal": "WRONG_SELECTED_DEFINITION",
        }
        for k in fields
    ]
...
def test_selected_definitions_come_from_retained_bundle_and_owner_binding(tmp_path):
    ...
    assert (
        KnowledgeChangeHistory.reopen(history.path).replay().receipt == replay.receipt
    )
