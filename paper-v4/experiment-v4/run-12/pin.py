"""Pin run-12 to one Core commit and write the manifest and the gate block.
...
reasons that are new since the v4.5 coordinate, the adapter's SUBJECT_NOT_NAMED
refusal messages, the subject census's own keys, and whether the adapter and
the skill moved.

Nothing here is typed by hand. A pack version that has not moved, or a Core-16
...
Five of the eighteen change entries are Core's and carried from run-11. Their
landed facts are read at fixed commits, the v4.1 baseline for Core-12 and the
v4.2, v4.3, v4.4 and v4.5 coordinates for Core-11, Core-13, Core-14 and
Core-15, so a later Core change cannot be attributed to them:
CORE_16_PROJECTED_SUBJECT owns everything the adapter and the skill gain after
the v4.5 coordinate.

Core-16 adds no refusal reason either. The adapter derives a subject where the
formalizing statement names exactly one entity of the capture, and the census
reports where each subject came from, so the enum does not move and the census
keys do; ``pin_status`` is therefore the two byte comparisons against the v4.5
coordinate, not a subset check on an empty ``expected_reasons``, and the census
keys the pinned bytes carry are recorded beside it.

    .venv/bin/python paper-v4/experiment-v4/run-12/pin.py --commit <sha>
"""
...
RUN_ID = "run-12"
PROTOCOL_VERSION = "v4.6"
INTERFACE_ORDINAL = "12"
PRODUCER_WORKSPACE = f"private/paper-v4-v4-{RUN_ID}/producer"

# The v4.1 to v4.5 coordinates the earlier cells ran at. All five are fixed
...
# between the fourth and the fifth, and Core-16 owns everything after the fifth,
# so no reason is attributed to the wrong Core task at a later pin.
...
# The four dispositions Core-16 gives the subject axis. A subject the producer
# set is ``proposed``, one the adapter derived is ``projected``, a statement
# naming more than one capture entity is ``ambiguous``, and one naming none is
# ``unnamed``. The census function is where they have to appear; the pin reads
# that function's own bytes and records which of them are there.
CENSUS_KEYS = ("ambiguous", "projected", "proposed", "unnamed")
SUBJECT_CENSUS_DEF = "def _subject_census("

LANDED = "LANDED"
PENDING = "PENDING_AT_PIN"
CARRIED = "CARRIED_FROM_RUN_11"
...
def subject_census_keys(commit: str) -> list[str]:
    """Which of Core-16's census keys the subject census declares at ``commit``.

    Core-16 adds no refusal reason, so the enum cannot report it. What it does
    add that is readable in the file is the disposition each counted record is
    filed under. The pin reads the census function's own body and records the
    keys it finds there; before Core-16 lands that is an empty list, and the
    change entry says so rather than claiming the change.
    """

    source = _text(commit, DOCUMENT_ADAPTER)
    start = source.find(SUBJECT_CENSUS_DEF)
    if start < 0:
        raise PinRefusal(
            f"{DOCUMENT_ADAPTER} declares no subject census at {commit}"
        )
    end = source.find("\ndef ", start + 1)
    body = source[start:] if end < 0 else source[start:end]
    return sorted(name for name in CENSUS_KEYS if f'"{name}"' in body)
...
def pin(commit_argument: str, reading: Path) -> dict[str, object]:
    ...
    # Carried from run-11, itself carried from run-08. The entry records what
    ...
    # Carried from run-11, where run-09 had it as the change under test. Its
    # reasons are what the enum gained between the v4.2 and the v4.3
    # coordinates, both fixed commits, so nothing Core-16 lands can be read as
    # Core-13's.
    ...
    # Carried from run-11, where run-10 had it as the change under test. Its
    # reason and both of its ride-alongs are read between the v4.3 and the v4.4
    # coordinates, all fixed commits, so nothing Core-16 lands can be read as
    ...
    # Carried from run-11, where it was the change under test. Its two byte
    # comparisons are now read between the v4.4 and the v4.5 coordinates, both
    # fixed commits, because Core-16 touches the same two files and a
    # comparison against the pin would report Core-16's bytes as Core-15's.
    # Its expectation is empty, so no enum check is possible either way and
    # the entry says so instead of carrying a vacuous subset result.
    ...
    # the v4.5 coordinate belongs to it. It adds no refusal reason either, so
    # the enum cannot report it: the pin reads the two files' bytes, the
    # SUBJECT_NOT_NAMED message text and the subject census's own keys, and
    # pin_status is the two byte comparisons rather than a subset check on an
    # empty expectation. The census keys are recorded beside it because they
    # are what the change adds that is readable in the file.
    projected = changes["CORE_16_PROJECTED_SUBJECT"]
    projected["reasons"] = sorted(at_commit - at_v4_5)
    projected_messages = subject_not_named_messages(commit)
    ...
    projected["census_keys"] = list(CENSUS_KEYS)
    projected["census_keys_at_pin"] = subject_census_keys(commit)
    projected["census_keys_present"] = projected["census_keys_at_pin"] == list(
        CENSUS_KEYS
    )
    projected["pin_status"] = (
        LANDED if all(projected["observed"].values()) else PENDING
    )
    projected["reasons_added"] = projected["reasons"] != []

    pending = [projected["core_task"]] if projected["pin_status"] != LANDED else []
    for carried in (derivation, subject, modality):
        if not carried["still_present_at_pin"]:
            pending.append(carried["core_task"])
    gate["status"] = (
        "PINNED_TO_THE_V4_6_CORE_COORDINATE"
        if not pending
    ...
    return {
        ...
        "core_16_adapter_moved": projected["adapter"]["moved"],
        "core_16_adapter_messages_moved": projected["adapter"]["messages_moved"],
        "core_16_skill_moved": projected["skill"]["moved"],
        "core_16_census_keys_at_pin": projected["census_keys_at_pin"],
