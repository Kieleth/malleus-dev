"""Pin run-17 to one Core commit and write the manifest and the gate block.
...
v4.9 changes nothing in Core. This cell pins the coordinate run-16 pinned, the
v4.8 commit, so all eight of the twenty-four change entries that are Core's are
carried from run-16 and every one of them reads as landed at a fixed coordinate:
...
Core-18 is carried the way run-16 carried Core-17, at fixed commits, and it is
...
run-16 are frozen and none of them is edited.

    .venv/bin/python paper-v4/experiment-v4/run-17/pin.py --commit <sha>
"""
...
RUN_ID = "run-17"
PROTOCOL_VERSION = "v4.9"
INTERFACE_ORDINAL = "17"
...
def pin(commit_argument: str, reading: Path) -> dict[str, object]:
    ...
    # Carried from run-16, itself carried from run-08. The entry records what
    ...
    # Carried from run-16, where run-09 had it as the change under test. Its
    ...
    # Carried from run-16, where run-10 had it as the change under test. Its
    ...
    # Carried from run-16, where run-11 had it as the change under test. Its
    ...
    # Carried from run-16, where run-12 had it as the change under test. Its
    ...
    # Carried from run-16, itself carried from run-13, where it was the change
