...
# reinterpreted, and neither is run-04; run-05 is an added cell
# at the coordinate run-04 opened.
V4_CELLS = (
    ("run-02", "opus", "ADMITTED_AND_REPLAYED"),
    ("run-03", "sonnet", "REFUSED_AFTER_DIAGNOSTIC_BUDGET"),
)

# The exact bytes the two closed v4 cells left in the repository. Run-05
...
def test_run_05_is_the_second_v4_1_cell_with_the_model_as_the_only_variable() -> (
    None
):
    contract = _contract()
    ...
    assert contract["run_id"] == "run-05"
    assert contract["supersedes"] == (
        "NOTHING_RUN_05_IS_AN_ADDED_CELL_OF_THE_V4_1_ITERATION"
    )
...
def test_the_change_list_names_every_difference_from_the_v4_cells() -> None:
    ...
    # written in run-04's compiler and run-05's is that file with the run id
...
def test_producer_record_names_sonnet_by_id_and_run_04s_harness_and_boundary() -> (
    None
):
...
def test_interface_coordinates_are_new_and_reuse_no_earlier_run() -> None:
    ...
    earlier = [
        ...
        for path in (RUN_01, RUN_02, RUN_03, RUN_04)
    ]

    assert coordinates == {
        "capture_id": "capture:paper-v4:yu-2025:v4:5",
        "plan_id": "plan:paper-v4:yu-2025:v4:5",
...
def test_spawn_message_is_run_04s_message_with_only_the_run_id_changed() -> None:
    ...
    assert here == run_04.replace("run-04", "run-05")
...
def test_the_paper_ledger_records_the_second_v4_1_cell() -> None:
    ledger = PAPER_LEDGER.read_text(encoding="utf-8")
    entry = ledger.split("### E-0126,")[-1]

    assert "### E-0126," in ledger
    for phrase in (
        "run-05",
        "v4.1",
        "run-04",
