"""Pin run-09 to one Core commit and write the manifest and the gate block.
...
Nothing here is typed by hand. A pack version that has not moved, or a Core-13
...
Two of the thirteen change entries are carried from run-08 and are not this
cell's expectations. Their landed facts are read at the v4.2 coordinate, which
is a commit and therefore fixed, so a later Core change cannot be attributed to
them: SUBJECT_ELEMENT owns everything the enum and the packs gained after that
commit.

    .venv/bin/python paper-v4/experiment-v4/run-09/pin.py --commit <sha>
"""
...
# The v4.1 coordinate the four v4.1 cells ran at, and the v4.2 coordinate run-08
# ran at. Both are fixed commits: the carried Core-12 entry reads what the enum
# gained between them, and SUBJECT_ELEMENT reads what it gained after the
# second, so no reason is attributed to the wrong Core task at a later pin.
...
def _moved_since_reference(declared: list[dict[str, str]]) -> dict[str, object]:
    """Which declared inputs carry other bytes than the v4.2 cell of record's.
    ...
    research pack and the skill move by design in v4.3; anything else that moved
    is a fact this run records rather than a change it claims.
    """
    reference = {
...
def pin(commit_argument: str, reading: Path) -> dict[str, object]:
    ...
    # Carried from run-08. The entry records what v4.2 landed and what the pinned
    # commit now carries, never a v4.3 expectation: research moves again here,
    # under SUBJECT_ELEMENT.
    ...
    # Carried from run-08. Its reasons are what the enum gained between its own
    # baseline and the v4.2 coordinate, both fixed commits; the pin only checks
    # that they are still in force here.
    ...
    # This cell's Core change. Everything the packs and the enum gained after the
    # v4.2 coordinate belongs to it.
    subject = changes["SUBJECT_ELEMENT"]
    subject["versions"] = {
        name: observed[name] for name in sorted(subject["expected_versions"])
    }
    subject["reasons"] = sorted(at_commit - set(refusal_reasons(subject["baseline_commit"])))
    subject["pin_status"] = (
        LANDED
    ...
    pending = [subject["core_task"]] if subject["pin_status"] != LANDED else []
    if not derivation["still_present_at_pin"]:
        pending.append(derivation["core_task"])
    gate["status"] = (
        "PINNED_TO_THE_V4_3_CORE_COORDINATE"
        if not pending
