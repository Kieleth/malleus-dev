"""Expand one file of type sets into the exhaustive run-09 query binding.
...
What v4.3 adds is reach. Run-08's graph held 131 observations and 85 claims and
not one relation from any of them to a feature, a sample, a campaign or another
claim, so a relation-only binding returned eight rows over four questions while
the answers sat in the graph as unattached records (E-0138, and the v4.2 RCA
sections 2 and 4). Two more case kinds close that, both type-only and
value-blind like the first:

* ``ENTITY`` names one record type and its projected fields. Every admitted
  record of that type is a row whose witness is itself.
* ``SUBJECT`` names a source-asserted record type and a subject entity type.
  Every record of the first whose ``subject`` resolves to a record of the second
  is a row, projecting both.

The expansion stays mechanical and the evaluator's one judgement stays the type
set per question: RELATION as before, ENTITY for every type in the set, and
SUBJECT for every ordered pair of a subject-bearing type in the set with an
entity type in the set. ``cases_sha256`` still digests the queries alone, so the
...
BINDING_SCHEMA = "malleus.paper-v4.native-query-binding/v3"
...
# carries this slot is a SUBJECT case's first type.
...
BOUND_BY = (
    ...
    " every one of them type-only: one ENTITY case per type, one RELATION case"
    " per ordered pair of those types under every relation type on the surface,"
    " and one SUBJECT case per ordered pair of a subject-bearing type with an"
    " entity type. Each type projects its non-housekeeping slots. No"
    " population, admission, replay, row or row count existed when the type"
    " sets were written, so no binding here was selected against a result."
    " Run-04's binding was revised twice after its rows were seen (deep sweep"
    " D-01); run-08 closed that and reached nothing but relations; this reaches"
...
def _cases(
    *,
    ...
    expected = (
        len(types) + len(types) * len(types) * len(relations) + len(bearing) * len(entities)
    )
    ...
    *, surface_source: bytes, type_sets: dict[str, list[str]], replay_receipt: str
    ...
    return {
        ...
        "expansion": {
            ...
            "rule": (
                "one ENTITY case per type in a question's set; every ordered"
                " pair of those types under every relation type on the surface"
                " as a RELATION case; every ordered pair of a subject-bearing"
                " type in the set with an entity type in the set as a SUBJECT"
                " case; each type projecting its non-housekeeping slots"
            ),
