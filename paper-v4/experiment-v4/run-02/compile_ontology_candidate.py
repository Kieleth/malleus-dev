"""The run-02 ontology gate the parent runs between producer attempts.

The compiler sources come from the producer workspace and are checked against
run-02's own frozen manifest. Refusals are written through unchanged: ``detail``
is the exception's own text, so the aggregate grounding diagnostic reaches the
producer with its complete sorted missing set, not one subject at a time.
"""
