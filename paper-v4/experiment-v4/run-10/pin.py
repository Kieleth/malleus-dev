"""Pin run-10 to one Core commit and write the manifest and the gate block.
...
reasons that are new since the v4.3 coordinate, the elaborator's INVALID_RANGE
refusal messages, and whether the skill moved.

Nothing here is typed by hand. A pack version that has not moved, or a Core-14
...
Three of the fifteen change entries are Core's and carried from run-09. Their
landed facts are read at fixed commits, the v4.1 baseline for Core-12 and the
v4.2 and v4.3 coordinates for Core-11 and Core-13, so a later Core change cannot
be attributed to them: CORE_14_MODALITY_SOURCE_OF_TRUTH owns everything the enum,
the elaborator and the skill gain after the v4.3 coordinate.

    .venv/bin/python paper-v4/experiment-v4/run-10/pin.py --commit <sha>
"""
...
INTERFACE_ORDINAL = "10"
PRODUCER_WORKSPACE = f"private/paper-v4-v4-{RUN_ID}/producer"

# The v4.1, v4.2 and v4.3 coordinates the earlier cells ran at. All three are
# fixed commits: the carried Core-12 entry reads what the enum gained between
# the first two, the carried Core-13 entry what it gained between the second and
# the third, and Core-14 owns everything after the third, so no reason is
# attributed to the wrong Core task at a later pin.
...
def _moved_since_reference(declared: list[dict[str, str]]) -> dict[str, object]:
    """Which declared inputs carry other bytes than the v4.3 cell of record's.
    ...
    skill moves by design in v4.4; anything else that moved is a fact this run
...
def pin(commit_argument: str, reading: Path) -> dict[str, object]:
    ...
    # Carried from run-09, itself carried from run-08. The entry records what
    ...
    # Carried from run-09, where it was the change under test. Its reasons are
    # what the enum gained between the v4.2 and the v4.3 coordinates, both fixed
    # commits, so nothing Core-14 lands can be read as Core-13's.
    ...
    # This cell's Core change. Everything the enum, the elaborator and the skill
    # gain after the v4.3 coordinate belongs to it.
    ...
    modality["pin_status"] = (
        LANDED if set(modality["expected_reasons"]) <= at_commit else PENDING
    )
    # The ride-alongs carry no semantic change and do not decide pin_status,
    # but the cell is not ready for a producer until they land: the iteration's
    # falsifiers include the two returns recurring, and a producer that runs
    # before the elaborator's message moves cannot observe either.
    ...
    pending = (
        [modality["core_task"]]
        if modality["pin_status"] != LANDED or not modality["ride_alongs_landed"]
        else []
    )
    ...
    gate["status"] = (
        "PINNED_TO_THE_V4_4_CORE_COORDINATE"
        if not pending
