...
# Nothing moved. This cell runs run-04's harness at run-04's Core coordinate
# with the producer model changed, so all eight declared inputs carry run-04's
# digests. The empty set is asserted rather than assumed.
...
# reinterprets them, and nothing here supersedes run-04 and run-05 either.
# Run-06 is an added cell at the coordinate run-04 opened.
V4_CELLS = (
    ("run-02", "opus", "ADMITTED_AND_REPLAYED"),
    ("run-03", "sonnet", "REFUSED_AFTER_DIAGNOSTIC_BUDGET"),
)

# The exact bytes the two closed v4 cells left in the repository. Run-06
# changes neither the harness nor the Core coordinate, only the model, so both
# closed runs must read the same after this one exists. The open v4.1 cells are
# not pinned here: no producer has finished in them and an artifact may land at
# any time.
...
def test_run_06_is_the_third_v4_1_cell_with_the_model_as_the_only_variable() -> (
    None
):
    contract = _contract()
    ...
    assert contract["run_id"] == "run-06"
    assert contract["supersedes"] == (
        "NOTHING_RUN_06_IS_AN_ADDED_CELL_OF_THE_V4_1_ITERATION"
    )
    ...
    assert scope["matrix_cell"] == "THIRD_OF_V4_1"
    assert scope["first_cell_of_v4_1"] == "run-04"
    assert scope["first_cell_of_v4_1_path"] == "paper-v4/experiment-v4/run-04"
    assert scope["v4_1_cells_preceding"] == ["run-04", "run-05"]
    assert scope["v4_cells"] == ["run-02", "run-03"]
    assert scope["model_matched_cell"] == "NONE_NO_EARLIER_CELL_USED_HAIKU"
    assert scope["variable"] == "PRODUCER_MODEL_ONLY"
    assert scope["harness"] == "IDENTICAL_TO_RUN_04"
...
def test_the_change_list_names_every_difference_from_the_v4_cells() -> None:
    ...
    # The change list is the iteration's, not this cell's. The surface change was
    # written in run-04's compiler and run-06's is that file with the run id
    # substituted, so the subject stays the file where the change landed.
...
def test_producer_record_names_haiku_by_id_and_run_04s_harness_and_boundary() -> (
    None
):
    ...
    # The model is the variable. Every other key of the producer block is
    # run-04's, byte for byte, and the three model keys are the only difference.
    varying = {"requested_model", "model_family", "model_id"}
    assert set(producer) == set(run_04)
    assert {key: producer[key] for key in producer if key not in varying} == {
        key: run_04[key] for key in run_04 if key not in varying
    }
    assert run_04["requested_model"] == "opus"
    assert run_04["model_family"] == "Claude Opus 5"
    assert run_04["model_id"] == "claude-opus-5"
...
def test_the_eight_declared_inputs_are_run_04s_at_the_same_digests() -> None:
    """No input moved. The model is the only variable between the two cells."""

    run_04 = json.loads((RUN_04 / "producer-input-manifest.json").read_bytes())
    prior = {item["name"]: item["sha256"] for item in run_04["declared_inputs"]}
    ...
    assert _manifest()["declared_inputs"] == run_04["declared_inputs"]
...
def test_interface_coordinates_are_new_and_reuse_no_earlier_run() -> None:
    ...
    earlier = [
        ...
        for path in (RUN_01, RUN_02, RUN_03, RUN_04, RUN_05)
    ]

    assert coordinates == {
        "capture_id": "capture:paper-v4:yu-2025:v4:6",
        "plan_id": "plan:paper-v4:yu-2025:v4:6",
    ...
    assert _manifest()["producer_workspace"] == (
        "private/paper-v4-v4-run-06/producer"
    )
...
def test_spawn_message_is_run_04s_message_with_only_the_run_id_changed() -> None:
    here = SPAWN_MESSAGE.read_text(encoding="utf-8")
    run_04 = (RUN_04 / "spawn-message.md").read_text(encoding="utf-8")

    assert here == run_04.replace("run-04", "run-06")
...
def test_the_active_gate_collects_run_06() -> None:
...
def test_the_paper_ledger_records_the_third_v4_1_cell() -> None:
    ledger = PAPER_LEDGER.read_text(encoding="utf-8")
    entry = ledger.split("### E-0127,")[-1]

    assert "### E-0127," in ledger
    for phrase in (
        "run-06",
        "v4.1",
        "run-04",
