"""Pin run-20 to one Core commit and write the manifest and the gate block.
...
Core-15, Core-16, Core-17 and Core-18, and the two entries carried from run-19
are read again between the v4.8 coordinate and that same commit. Core-19 and
Core-20 both read LANDED there, as run-19's pin read them. Nothing else here
owns what the adapter or the skill gains after the v4.8 coordinate.

Core-18 is carried the way run-19 carried it, at fixed commits, and it is not
...
run-19 are frozen and none of them is edited.

    .venv/bin/python paper-v4/experiment-v4/run-20/pin.py --commit <sha>
"""
...
# gained between the seventh and the eighth, which is the coordinate run-19
# pinned and the baseline the two entries carried from run-19 are read against.
...
# The two statuses the Core entries carried from run-19 can read. Both landed
...
# The status of every entry this cell carries unchanged from run-19. Two of them
# are Core-19 and Core-20, which read LANDED or PENDING_AT_PIN instead, off the
# bytes between the v4.8 coordinate and the commit run-19 pinned, which is the
# commit this cell pins.
CARRIED = "CARRIED_FROM_RUN_19"
...
def _moved_since_reference(declared: list[dict[str, str]]) -> dict[str, object]:
    ...
    records which inputs moved against run-19's, and none is expected to have.
    """
    reference = {
...
def pin(commit_argument: str, reading: Path) -> dict[str, object]:
    ...
    # Carried from run-19, itself carried from run-08. The entry records what
    ...
    # Carried from run-19, where run-09 had it as the change under test. Its
    ...
    # Carried from run-19, where run-10 had it as the change under test. Its
    ...
    # Carried from run-19, where run-11 had it as the change under test. Its
    ...
    # Carried from run-19, where run-12 had it as the change under test. Its
    ...
    # Carried from run-19, itself carried from run-13, where it was the change
    ...
    # and run-14, where it was the change under test. Its two byte comparisons, its enum
    ...
    # Carried from run-19, and the entry the producer reads. Core-20 adds no
