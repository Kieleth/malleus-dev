"""Pin run-18 to one Core commit and write the manifest and the gate block.
...
run-17 are frozen and none of them is edited.

    .venv/bin/python paper-v4/experiment-v4/run-18/pin.py --commit <sha>
"""
...
# run-17's coordinate. The three block labels are string constants inside the
...
def pin(commit_argument: str, reading: Path) -> dict[str, object]:
    ...
    # Carried from run-17, itself carried from run-08. The entry records what
    ...
    # Carried from run-17, where run-09 had it as the change under test. Its
    ...
    # Carried from run-17, where run-10 had it as the change under test. Its
    ...
    # Carried from run-17, where run-11 had it as the change under test. Its
    ...
    # Carried from run-17, where run-12 had it as the change under test. Its
    ...
    # Carried from run-17, itself carried from run-13, where it was the change
    ...
    # reached run-17's runner as a raw ValueError. It is read between run-17's
