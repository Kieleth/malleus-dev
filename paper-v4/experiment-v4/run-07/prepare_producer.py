"""Build the run-07 producer workspace from the bytes at the recorded commit.
...
The checker owns ``check/`` and the producer never writes there, so this builder
...
def prepare(reading: Path, output: Path) -> dict[str, object]:
    ...
    if any(check.iterdir()):
        raise ProducerPreparationRefusal("the checker directory must start empty")
