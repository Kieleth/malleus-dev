"""Pin run-23 to one Core commit and write the manifest and the gate block.
...
Core-15, Core-16, Core-17 and Core-18, and the two entries carried from run-22
are read again between the v4.8 coordinate and that same commit. Core-19 and
Core-20 both read LANDED there, as run-22's pin read them. Nothing else here
owns what the adapter or the skill gains after the v4.8 coordinate.

Core-18 is carried the way run-22 carried it, at fixed commits, and it is not
...
run-22 are frozen and none of them is edited.

    .venv/bin/python paper-v4/experiment-v4/run-23/pin.py --commit <sha>
"""
...
# gained between the seventh and the eighth, which is the coordinate run-22
# pinned and the baseline the two entries carried from run-22 are read against.
...
# The two statuses the Core entries carried from run-22 can read. Both landed
...
# The status of every entry this cell carries unchanged from run-22. Two of them
# are Core-19 and Core-20, which read LANDED or PENDING_AT_PIN instead, off the
# bytes between the v4.8 coordinate and the commit run-22 pinned, which is the
# commit this cell pins.
CARRIED = "CARRIED_FROM_RUN_22"
...
def pin(commit_argument: str, reading: Path) -> dict[str, object]:
    ...
    # Carried from run-22, itself carried from run-08. The entry records what
    ...
    # Carried from run-22, where run-09 had it as the change under test. Its
    ...
    # Carried from run-22, where run-10 had it as the change under test. Its
    ...
    # Carried from run-22, where run-11 had it as the change under test. Its
    ...
    # Carried from run-22, where run-12 had it as the change under test. Its
    ...
    # Carried from run-22, itself carried from run-13, where it was the change
    ...
    # under test. Its two byte comparisons, its enum
    ...
    # Carried from run-22, and run-18's own before that. Core-19 makes the
    ...
    # Carried from run-22, and the entry the producer reads. Core-20 adds no
