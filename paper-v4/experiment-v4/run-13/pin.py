"""Pin run-13 to one Core commit and write the manifest and the gate block.
...
reasons that are new since the v4.6 coordinate, the adapter's SUBJECT_NOT_NAMED
refusal messages, the subject census's own outcome keys, and whether the
adapter and the skill moved.

Nothing here is typed by hand. A pack version that has not moved, or a Core-17
...
Six of the twenty change entries are Core's and carried from run-12. Their
landed facts are read at fixed commits, the v4.1 baseline for Core-12 and the
v4.2 to v4.6 coordinates for Core-11, Core-13, Core-14, Core-15 and Core-16, so
a later Core change cannot be attributed to them:
CORE_17_PROJECTION_WITHDRAWN owns everything the adapter and the skill gain or
lose after the v4.6 coordinate.

Core-17 is a removal and adds no refusal reason either. The adapter stops
deriving a subject and the census reports ``attachable`` where it reported
``projected``, so the enum does not move and the census keys do; ``pin_status``
is therefore the two byte comparisons against the v4.6 coordinate, not a subset
check on an empty ``expected_reasons``, and the census's own outcome keys at the
pinned commit are recorded beside it.

The census reader is not run-12's. Run-12's scanned the census function's body
for the four literals and found none of them, because the adapter names its
outcomes by module constant there, so run-12's frozen contract records
``census_keys_at_pin: []`` at a commit where Core-16 had landed. This one
resolves the adapter's own ``_SUBJECT_OUTCOMES`` tuple and records what it
finds. Run-12 is frozen and is not edited.

    .venv/bin/python paper-v4/experiment-v4/run-13/pin.py --commit <sha>
"""
...
RUN_ID = "run-13"
PROTOCOL_VERSION = "v4.7"
INTERFACE_ORDINAL = "13"
PRODUCER_WORKSPACE = f"private/paper-v4-v4-{RUN_ID}/producer"

# The v4.1 to v4.6 coordinates the earlier cells ran at. All six are fixed
...
# between the fifth and the sixth, and Core-17 owns everything after the sixth,
...
def _moved_since_reference(declared: list[dict[str, str]]) -> dict[str, object]:
    """Which declared inputs carry other bytes than the v4.6 cell of record's.
    ...
    skill moves by design in v4.7; anything else that moved is a fact this run
...
def pin(commit_argument: str, reading: Path) -> dict[str, object]:
    ...
    # Carried from run-12, itself carried from run-08. The entry records what
    ...
    # Carried from run-12, where run-09 had it as the change under test. Its
    # reasons are what the enum gained between the v4.2 and the v4.3
    # coordinates, both fixed commits, so nothing Core-17 lands can be read as
    # Core-13's.
    ...
    # Carried from run-12, where run-10 had it as the change under test. Its
    # reason and both of its ride-alongs are read between the v4.3 and the v4.4
    # coordinates, all fixed commits, so nothing Core-17 lands can be read as
    ...
    # Carried from run-12, where run-11 had it as the change under test. Its
    ...
    # Carried from run-12, where it was the change under test. Its two byte
    # comparisons are now read between the v4.5 and the v4.6 coordinates, both
    # fixed commits, because Core-17 rewrites the same adapter and the same
    # skill and a comparison against the pin would report Core-17's bytes as
    # Core-16's. Its census keys are read at the v4.6 coordinate for the same
    # reason: the four it added are what v4.6 carried, not what this pin does.
    ...
    # This cell's Core change, and a removal. Everything the adapter and the
    # skill gain or lose after the v4.6 coordinate belongs to it. It adds no
    # refusal reason and removes none, so the enum cannot report it either: the
    # pin reads the two files' bytes, the SUBJECT_NOT_NAMED message text and
    # the census's own outcome keys, and pin_status is the two byte comparisons
    # rather than a subset check on an empty expectation. The census keys are
    # recorded beside it because a withdrawal is only readable there: the four
    # that must be present and the one that must be gone.
    withdrawn = changes["CORE_17_PROJECTION_WITHDRAWN"]
    withdrawn["reasons"] = sorted(at_commit - at_v4_6)
    withdrawn_messages = subject_not_named_messages(commit)
    ...
    withdrawn["census_keys_at_pin"] = subject_census_keys(commit)
    withdrawn["census_keys_present"] = withdrawn["census_keys_at_pin"] == list(
        CENSUS_KEYS
    )
    withdrawn["projection_withdrawn_at_pin"] = (
        WITHDRAWN_CENSUS_KEY not in withdrawn["census_keys_at_pin"]
    )
    withdrawn["pin_status"] = (
        LANDED if all(withdrawn["observed"].values()) else PENDING
    )
    withdrawn["reasons_added"] = withdrawn["reasons"] != []

    pending = [withdrawn["core_task"]] if withdrawn["pin_status"] != LANDED else []
    for carried in (derivation, subject, modality):
        if not carried["still_present_at_pin"]:
            pending.append(carried["core_task"])
    gate["status"] = (
        "PINNED_TO_THE_V4_7_CORE_COORDINATE"
        if not pending
    ...
    return {
        ...
        "core_17_adapter_moved": withdrawn["adapter"]["moved"],
        "core_17_adapter_messages_moved": withdrawn["adapter"]["messages_moved"],
        "core_17_skill_moved": withdrawn["skill"]["moved"],
        "core_17_census_keys_at_pin": withdrawn["census_keys_at_pin"],
        "core_17_projection_withdrawn_at_pin": withdrawn[
            "projection_withdrawn_at_pin"
        ],
