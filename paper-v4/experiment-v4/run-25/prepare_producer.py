"""Build the run-25 producer workspace from the bytes at the recorded commit.
...
One declared input is not read at the Core commit. This cell's ninth input is
run-23's accepted ontology, which is a paper artefact frozen after the Core
commit and therefore absent from that tree. A declared input may name its own
``source_commit``; the builder reads it there and at no other coordinate, so the
bytes the producer consumes are still a commit git already holds. This is the
fixed-ontology control: the producer does not author an ontology and starts at
phase two.
"""
...
def prepare(reading: Path, output: Path) -> dict[str, object]:
    ...
    for item in manifest["declared_inputs"]:
        read = (
            ...
            else _git_show(item.get("source_commit", commit), item["source"])
        )
