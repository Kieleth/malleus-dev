"""Freeze run-25: copy the public set, measure the leak ladder, write the record.
...
admitted and the query has executed:
    ...
    PYTHONPATH=<core export>/src:$PWD:$PWD/src .venv/bin/python \\
        paper-v4/experiment-v4/run-25/freeze.py --ladder <ladder.py>
...
Two things it does not do. It does not write ``paper-v4/paper-ledger.md``: only
the overseer writes that file, and this cell's agent was told not to. And it
does not claim this cell's ontology as its producer's: ``ontology-run/`` holds
run-23's accepted bytes, staged here as the ninth declared input, and the result
record says so in ``ontology_source``, ``ontology_authored_by`` and
``ontology_source_commit``.
"""
...
def copy_public() -> None:
    ...
    shutil.copyfile(
        PRIVATE / "producer/inputs/ontology-run-23.yaml",
...
def freeze(ladder_script: Path) -> dict[str, object]:
    ...
    (PUBLIC / "withheld-artifacts.json").write_text(
        json.dumps(
            {
                "schema": "malleus.paper-v4.run-25-withheld-artifacts/v1",
                "run_id": "run-25",
                "reason": (
    ...
    prior = json.loads((RUN_23 / "ontology-run/result.json").read_bytes())
    result = {
        ...
        "ontology_source": "FIXED_RUN_23_ACCEPTED_ONTOLOGY",
        "ontology_authored_by": "run-23",
        "ontology_source_commit": FIXED_ONTOLOGY_COMMIT,
        "ontology_source_path": (
            "paper-v4/experiment-v4/run-23/ontology-run/ontology-01.yaml"
        ),
        "ontology_not_authored_here": (
            "This cell's producer authored no ontology. ontology-run/ holds"
            " run-23's accepted bytes, staged into the workspace as the ninth"
            " declared input and gated here as a phase-one attempt would be"
            " gated."
        ),
        "producer": {
            ...
            "ontology_authored": False,
            "phases": "POPULATION_ONLY",
            "diagnostic_returns": len(log["gate"]) - 1,
            "diagnostic_return_limit": 0,
            "fallback_used": False,
        ...
        "attempts": [
            {
                ...
                "ontology_path": "paper-v4/experiment-v4/run-25/ontology-run/ontology-01.yaml",
                "ontology_visibility": "PUBLIC",
                "diagnostic_path": (
                    "paper-v4/experiment-v4/run-25/ontology-run/attempt-01-diagnostic.json"
                ),
        ...
        "reproduces_run_23": {
            **gate["reproduces_run_23"],
            "run_23_accepted": prior["accepted"],
        },
        ...
        "next_boundary": (
            ...
            " paper-v4/evaluation-v4/run-25 and Luis's ratification are pending;"
    ...
    review.validate_review_input_manifest(
        ...
        (EVALUATION / "review-protocol-v3.json").read_bytes(),
    )
    ...
    witnesses = json.loads((PUBLIC / "query-trace-summary.json").read_bytes())[
        "witnesses_traced"
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({key: report[key] for key in ("public_ladder_max", "witnesses", "families")}, indent=2))
    ...
    Path("/private/tmp/claude-501/-Users-luis-Projects-malleus-dev/"
         "1e1b7a20-5bb3-486c-8880-9c4ee4c9be88/scratchpad/freeze-report.json").write_text(
