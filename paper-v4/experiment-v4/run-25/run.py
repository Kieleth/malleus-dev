...
RESULT_SCHEMA = "malleus.paper-v4.run-25-result/v1"
                ...
                "events": [
                    {
                        ...
                        # run-25 addition: the ontology this cell accepted for
                        # population is not this producer's. It is run-23's
                        # accepted ontology, given to this producer as a
                        # declared input, and the event says so rather than
                        # leaving the reader to infer it from the digest.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"run-25: {type(error).__name__}: {error}", file=sys.stderr)
        return 2
    return 0
