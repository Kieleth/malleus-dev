...
Rerunning the fixture reopens the same history and emits byte-identical evidence without changing the ledger. That reproduction holds at the commit carrying this manuscript revision. Four files under the shipped \code{malleus} package changed between the coordinate the document run is pinned to and this commit (the document adapter's aggregated locator diagnostics, the admission path's handling of list-valued properties, the population compiler, and the pack-grounding rite), and no file under the fixture did; the calibration and the document run therefore share the fixture and the protocol, not one byte-identical implementation.

The fixture is a conformance case, not a protocol rule. The plans are adopter-authored and committed with the fixture under \code{public\_population/plans/}; their format is the fixture's own, not a protocol rule. The state-version history profile is a fixture choice. Event population, semantic re-entry, external effects and a stable wire format are out of its scope. It is evidence about this mechanism only, and nothing here evaluates the chapter it transcribes, its method, or its results.
...
\paragraph{Text-layer damage is a typed gap, not a repair.} The reading is the PDF text layer as pypdf 6.16.2 extracts it. Where the PDF sets characters individually, the extractor emits a space between letters, so a name arrives as \code{S a t i s h C . S i n g h}. Twenty-seven of the 186 blocks carry a run like that, almost all in the byline, the acknowledgements and the reference list, and the abstract carries one inside a sentence the producer captured verbatim, spacing included. A producer bound to verbatim capture cannot record a damaged name as a name without reconstructing it, and reconstruction is invention, so the protocol records the field as a \code{REQUIRED\_FIELD\_ABSENT\_IN\_SOURCE} gap and moves on. The remedy is upstream, in the extraction that produces the reading, and re-extracting would change the digest every cell, manifest and review is pinned to; for this paper the damage stays declared, confined to bylines and references, and counted.
