...
$\downarrow$\\[-2pt]
disposal $\rightarrow$ reopen and replay $\rightarrow$ adopter-owned graph reads\\[2pt]
$\downarrow$\\[-2pt]
...
The experiment is pinned to \malleus{} Core commit \gitid{f9052b4783100203318d4a21a0236f3851218af1}, tree \gitid{39a1ab48b913abc26f975873792c639ee690e811}. The exact paper reproducer is local commit \gitid{8e818103e6867e326544123a30abe756bdd45117}, tree \gitid{455e91e3110d1789fb3db8c8a902bc2e87c4eb04}; it contains the v2 experiment, driver, and environment lock. This reproduction coordinate is outside the five experiment identity groups and is not claimed as a published artifact. The run uses the research ontology compiler, restricted template grammar, paper-local population bridge, private change-set composer, ledger admission, replay, and native graph reads. These are identified research seams, not stable public APIs or wire contracts.
...
Before population, the binding exposed the expressibility limits in Table~\ref{tab:expressibility}. These gaps were recorded and left unchanged.
...
\toprule
Question & Expressible & Missing semantics \\
\midrule
CQ1 & Method uses instrument & Campaign, network, acquisition, instrument count \\
CQ2 & Phenomenon occurs at feature; feature part of feature & Relative position such as beneath an axis \\
CQ3 & Quantitative observation, target, constituent, estimate status & None required by the frozen binding \\
CQ4 & Process components and observations & Hypothesis, preference, epistemic status, direction \\
\bottomrule
\end{tabular}
\end{table}
...
\item Adopter query binding: \digest{922e2c628a86bca22d761ebf6d453c9056ead8bdc5301e3c5dfb193db61368c1}.
\end{enumerate}
...
The Core baseline is commit \gitid{f9052b4783100203318d4a21a0236f3851218af1}, tree \gitid{39a1ab48b913abc26f975873792c639ee690e811}. The exact paper reproducer is local commit \gitid{8e818103e6867e326544123a30abe756bdd45117}, tree \gitid{455e91e3110d1789fb3db8c8a902bc2e87c4eb04}. The latter contains the v2 experiment, environment lock, and driver; it is not asserted to be pushed or public and remains outside the five experiment identity groups. The ignored source PDF must exist at the path named by \code{paper-v4/source/source-manifest.json}. Reproduction requires CPython 3.12.9 on macOS arm64, the hash-checked lock, and the private research seam from that exact paper checkout. Both output paths must be absent before execution. The complete commands are retained in the source bundle's \code{README.md}.
