...
Instantiate it at freeze by substituting `run-25`, `SELECTED_READING_TEXT_LAYER`,
one question id per question of the cell's frozen file, one row-count placeholder per question,
`7743`, `505` and every path below with the frozen
cell's own figures, and write the result to
`paper-v4/evaluation-v4/run-25/review-task.md`. Run-05's task carried
...
- Source support is judged once per distinct witness, 505 of
...
`paper-v4/evaluation-v4/run-25/review-input-manifest.json`, which also binds
...
- `paper-v4/experiment-v4/run-25/results/native-query-binding.json`, the type-only binding.
- `private/paper-v4-v4-run-25/query/query-result.json`, the rows.
- `paper-v4/experiment-v4/run-25/results/trace-summary.json`, provenance for every populated record, and
  `paper-v4/experiment-v4/run-25/results/query-trace-summary.json`, the same for the 505
...
Judge every returned row exactly once, in order: 153 rows for `CQ-T1-01`, 153 for `CQ-T1-02`, 207 for `CQ-T1-03`, 205 for
...
`CQ-C-05`,
7743 in all, over 505 distinct witnesses. Cite at
...
Copy `paper-v4/evaluation-v4/run-25/review-record.blank.md`, set `status` to
