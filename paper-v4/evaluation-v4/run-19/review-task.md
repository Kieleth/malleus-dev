# Malleus paper v4 run-19 source-grounded review task

Template, version 4. Instantiate it at freeze by substituting `run-19`,
`15`, `37`, `52`, `50`,
`154` and `54` with the frozen cell's own figures, and
write the result to `paper-v4/evaluation-v4/run-19/review-task.md`. Run-05's
...
`paper-v4/evaluation-v4/run-19/review-input-manifest.json`, which also binds every
...
- `paper-v4/experiment-v4/run-19/results/native-query-binding.json`, the type-only
  binding, expanded from the evaluator's type sets at ontology acceptance and
  before phase two existed.
- `private/paper-v4-v4-run-19/query/query-result.json`, the rows.
- `paper-v4/experiment-v4/run-19/results/trace-summary.json`, provenance for every
  populated record, and
  `paper-v4/experiment-v4/run-19/results/query-trace-summary.json`, the same for
  the 54 witnesses the returned rows use.
- `private/paper-v4-v4-run-19/ledger/retained-capture.json`, the retained capture.
- this task, the frozen protocol, the input manifest, and a copy of
  `paper-v4/evaluation-v4/run-19/review-record.run-19.blank.md`.
...
Judge every returned row exactly once, in order: 15 rows for CQ-01,
37 for CQ-02, 52 for CQ-03, 50 for CQ-04,
154 in all. Cite at least one reading block per row and per question.
Write each reason in your own words. Copy no source passage into the record and
add no numerical aggregate.

## Recording

Copy `paper-v4/evaluation-v4/run-19/review-record.run-19.blank.md`, set `status` to
