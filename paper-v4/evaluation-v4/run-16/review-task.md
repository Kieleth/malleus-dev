# Malleus paper v4 run-16 source-grounded review task

Template, version 4. Instantiate it at freeze by substituting `run-16`,
`36`, `112`, `144`, `141`,
`433` and `157` with the frozen cell's own figures, and
write the result to `paper-v4/evaluation-v4/run-16/review-task.md`. Run-05's
...
`paper-v4/evaluation-v4/run-16/review-input-manifest.json`, which also binds every
...
- `paper-v4/experiment-v4/run-16/results/native-query-binding.json`, the type-only
  binding, expanded from the evaluator's type sets at ontology acceptance and
  before phase two existed.
- `private/paper-v4-v4-run-16/query/query-result.json`, the rows.
- `paper-v4/experiment-v4/run-16/results/trace-summary.json`, provenance for every
  populated record, and
  `paper-v4/experiment-v4/run-16/results/query-trace-summary.json`, the same for
  the 157 witnesses the returned rows use.
- `private/paper-v4-v4-run-16/ledger/retained-capture.json`, the retained capture.
- this task, the frozen protocol, the input manifest, and a copy of
  `paper-v4/evaluation-v4/run-16/review-record.run-16.blank.md`.
...
Judge every returned row exactly once, in order: 36 rows for CQ-01,
112 for CQ-02, 144 for CQ-03, 141 for CQ-04,
...
Copy `paper-v4/evaluation-v4/run-16/review-record.run-16.blank.md`, set `status` to
