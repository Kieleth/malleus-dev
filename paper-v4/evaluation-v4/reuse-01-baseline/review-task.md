...
This is reuse-01-baseline's review task, written by
...
`paper-v4/evaluation-v4/reuse-01-baseline/review-input-manifest.json`, which also binds
...
- `private/paper-v4-reuse-01-baseline/producer/work/answers.json`, the producer's answer file. This is what
  you read where a graph cell's reviewer reads a query result. Its grammar is
  `paper-v4/experiment-v4/reuse-01-baseline/answer-file-schema.json`, and it has
...
- `paper-v4/experiment-v4/reuse-01-baseline/spawn-message.md`, the message the producer was given.
- this task, the frozen protocol, the input manifest, the per-question review
  blocks in `paper-v4/evaluation-v4/reuse-01-baseline/`, and a copy of
  `paper-v4/evaluation-v4/reuse-01-baseline/review-record.blank.md`.
...
whose statement carries that item; or write `null` there and exactly one
...
On this surface the protocol adds: A prose answer has no slot and no record,
so NOT_CAPTURED reads there as 'the answer set does not state it and
declares no NO_ANSWER_IN_SOURCE for it'. UNREACHED_RECORD and
LOCATOR_NOT_RESOLVABLE do not arise on that surface.
...
Judge every claim exactly once, in order: 3 claims for `CQ-B-T1-01`, 3 for `CQ-B-T1-02`, 4 for `CQ-B-T1-03`, 4 for
...
`CQ-B-C-05`,
102 in all, over 102 distinct claims. Cite at
...
It was written for a graph surface and it binds this one the same way.
Read `returned row` as `cited claim` and `record, subject or relation`
as `the thing the claim is about`. A claim that carries a same-named
element for a different subject does not name the semantic: write
`row_index: null` and the absence cause. A control whose subject the
reading says nothing about reads `NONE` unless a claim carries a
required element about that control's own subject.
...
Copy `paper-v4/evaluation-v4/reuse-01-baseline/review-record.blank.md`, set `status` to
...
The per-question review blocks in `paper-v4/evaluation-v4/reuse-01-baseline/` carry each
