...
Instantiate it at freeze by substituting `run-22-v413`, `SELECTED_READING_TEXT_LAYER`,
one question id per question of the cell's frozen file, one row-count placeholder per question,
`6960`, `457` and every path below with the frozen
cell's own figures, and write the result to
`paper-v4/evaluation-v4/run-22-v413/review-task.md`. Run-05's task carried
...
- Source support is judged once per distinct witness, 457 of
...
`paper-v4/evaluation-v4/run-22-v413/review-input-manifest.json`, which also binds
...
- `paper-v4/experiment-v4/run-22/rebind-v4.13/native-query-binding.json`, the type-only binding.
- `private/paper-v4-v4-run-22/rebind-v4.13/query-result.json`, the rows.
- `paper-v4/experiment-v4/run-22/results/trace-summary.json`, provenance for every populated record, and
  `private/paper-v4-v4-run-22/rebind-v4.13/trace-summary.json`, the same for the 457
...
- this task, the frozen protocol, the input manifest, and a copy of
  `paper-v4/evaluation-v4/run-22-v413/review-record.blank.md`.
...
Judge every returned row exactly once, in order: 142 rows for `CQ-T1-01`, 142 for `CQ-T1-02`, 209 for `CQ-T1-03`, 209 for
...
`CQ-C-05`,
6960 in all, over 457 distinct witnesses. Cite at
...
Copy `paper-v4/evaluation-v4/run-22-v413/review-record.blank.md`, set `status` to
