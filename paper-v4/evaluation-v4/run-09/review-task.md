# Malleus paper v4 run-09 source-grounded review task

Template, version 3. Instantiate it at freeze by substituting `run-09`,
`210`, `547`, `359`, `350`,
`1466` and `343` with the frozen cell's own figures, and
write the result to `paper-v4/evaluation-v4/run-09/review-task.md`. Run-05's
...
`paper-v4/evaluation-v4/run-09/review-input-manifest.json`, which also binds every
...
- `paper-v4/experiment-v4/run-09/results/native-query-binding.json`, the type-only
  binding, expanded from the evaluator's type sets at ontology acceptance and
  before phase two existed.
- `private/paper-v4-v4-run-09/query/query-result.json`, the rows.
- `paper-v4/experiment-v4/run-09/results/trace-summary.json`, provenance for every
  populated record, and
  `paper-v4/experiment-v4/run-09/results/query-trace-summary.json`, the same for
  the 343 witnesses the returned rows use.
- `private/paper-v4-v4-run-09/ledger/retained-capture.json`, the retained capture.
- this task, the frozen protocol, the input manifest, and a copy of
  `paper-v4/evaluation-v4/run-09/review-record.run-09.blank.md`.
...
**Derivation locality, every row.** Say whether the block of the assertion that
formalizes the row's central record is among the blocks that formalize what the
row attaches it to: for `RELATION`, at least one endpoint; for `SUBJECT`, the
subject record; for `ENTITY`, the record's own blocks, which makes the token
`DERIVATION_LOCAL` by construction and is recorded anyway so the axis is
complete. Write `DERIVATION_LOCAL` or `DERIVATION_NON_LOCAL` as the next token of
that row's `rationale`, then the reason in your own words. Run-04 carried one
assertion with thirty-six formalization targets for twelve relations whose
endpoints it names nowhere: the pairings held on the reading and the evidence
pointers did not. Judge `source_support` on the reading as this task directs; the
locality token is a separate statement of fact and never a reason to downgrade
support on its own.
...
carried the subject as text inside `quantity_kind`. `RELATION` rows carry two
tokens and no third.
...
Judge every returned row exactly once, in order: 210 rows for CQ-01,
547 for CQ-02, 359 for CQ-03, 350 for CQ-04,
1466 in all. Cite at least one reading block per row and per question.
Write each reason in your own words. Copy no source passage into the record and
add no numerical aggregate.

## Recording

Copy `paper-v4/evaluation-v4/run-09/review-record.run-09.blank.md`, set `status` to
