# Malleus paper v4 run-02 source-grounded review task

Status: the method was frozen before the run-02 producer ran, at
...
`paper-v4/evaluation-v4/review-input-manifest.json`, which also binds every input
...
- `paper-v4/experiment-v4/run-02/results/native-query-binding.json`, the type-only
  binding written after the replay was frozen.
- `private/paper-v4-v4-run-02/query/query-result.json`, the rows. It is private
  because the producer put verbatim source sentences into record properties.
- `paper-v4/experiment-v4/run-02/results/trace-summary.json`, provenance for every
  populated record, and
  `paper-v4/experiment-v4/run-02/results/query-trace-summary.json`, the same for
  the 126 witnesses the returned rows use.
- `private/paper-v4-v4-run-02/ledger/retained-capture.json`, the retained capture,
...
Copy `review-record.blank.md`, set `status` to `PRELIMINARY_COMPLETE`, bind the
