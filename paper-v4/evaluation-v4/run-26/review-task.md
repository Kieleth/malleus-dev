# Malleus paper v4 source-grounded review task, protocol v3.2

This is run-26's review task, written by
...
(`sha256:5dfd59f4aa479dd72738e2eb55653ee3e84e60886de4cccbcc0a8e66d06e55cd`). It is not edited for this
...
- **A third evidence surface kind, `IN_CONTEXT_ANSWER_SET`.** This cell does not
  use it. Your surface is `SELECTED_READING_TEXT_LAYER`, as the manifest declares.
...
`paper-v4/evaluation-v4/run-26/review-input-manifest.json`, which also binds
...
- `paper-v4/experiment-v4/run-26/results/native-query-binding.json`, the type-only binding.
- `private/paper-v4-v4-run-26/query/query-result.json`, the rows.
- `paper-v4/experiment-v4/run-26/results/trace-summary.json`, provenance for every populated record, and
  `paper-v4/experiment-v4/run-26/results/query-trace-summary.json`, the same for the 547
...
Judge every returned row exactly once, in order: 148 rows for `CQ-T1-01`, 148 for `CQ-T1-02`, 247 for `CQ-T1-03`, 97 for
...
`CQ-C-05`,
7039 in all, over 547 distinct witnesses. Cite at
...
(`sha256:2f897a0f864b595b4562f7335152907091d7114915a60377404d6e5dd7473c9c`), whose own status line says it
...
in order. Reproduce it in `paper-v4/evaluation-v4/run-26/review-checklist.md` beside the record, with a tick and one line
...
Copy `paper-v4/evaluation-v4/run-26/review-record.blank.md`, set `status` to
...
The record's schema is `malleus.paper-v4.source-grounded-review/v3.2` and its key set is exact: the
checklist ticks go in `paper-v4/evaluation-v4/run-26/review-checklist.md`, never in it.
