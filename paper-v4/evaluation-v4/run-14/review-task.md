# Malleus paper v4 run-14 source-grounded review task

Template, version 4. Instantiate it at freeze by substituting `run-14`,
`190`, `195`, `323`, `211`,
`919` and `389` with the frozen cell's own figures, and
write the result to `paper-v4/evaluation-v4/run-14/review-task.md`. Run-05's
...
`paper-v4/evaluation-v4/run-14/review-input-manifest.json`, which also binds every
...
- `paper-v4/experiment-v4/run-14/results/native-query-binding.json`, the type-only
  binding, expanded from the evaluator's type sets at ontology acceptance and
  before phase two existed.
- `private/paper-v4-v4-run-14/query/query-result.json`, the rows.
- `paper-v4/experiment-v4/run-14/results/trace-summary.json`, provenance for every
  populated record, and
  `paper-v4/experiment-v4/run-14/results/query-trace-summary.json`, the same for
  the 389 witnesses the returned rows use.
- `private/paper-v4-v4-run-14/ledger/retained-capture.json`, the retained capture.
- this task, the frozen protocol, the input manifest, and a copy of
  `paper-v4/evaluation-v4/run-14/review-record.run-14.blank.md`.
...
Judge every returned row exactly once, in order: 190 rows for CQ-01,
195 for CQ-02, 323 for CQ-03, 211 for CQ-04,
919 in all. Cite at least one reading block per row and per question.
Write each reason in your own words. Copy no source passage into the record and
add no numerical aggregate.

## Recording

Copy `paper-v4/evaluation-v4/run-14/review-record.run-14.blank.md`, set `status` to
