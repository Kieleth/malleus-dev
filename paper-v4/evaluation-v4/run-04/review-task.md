# Malleus paper v4 run-04 source-grounded review task

Status: the method was frozen before the run-04 producer ran, at
...
`paper-v4/evaluation-v4/run-04/review-input-manifest.json`, which also binds every input
...
- `paper-v4/experiment-v4/run-04/results/native-query-binding.json`, the type-only
  binding written after the replay was frozen.
- `private/paper-v4-v4-run-04/query/query-result.json`, the rows. It is private
  because the producer put verbatim source sentences into record properties.
- `paper-v4/experiment-v4/run-04/results/trace-summary.json`, provenance for every
  populated record, and
  `paper-v4/experiment-v4/run-04/results/query-trace-summary.json`, the same for
  the 114 witnesses the returned rows use.
- `private/paper-v4-v4-run-04/ledger/retained-capture.json`, the retained capture,
  read only to resolve a locator to a reading block.
- this task, the frozen protocol, the input manifest, and a copy of
  `paper-v4/evaluation-v4/run-04/review-record.blank.md`.
...
Judge every returned row exactly once, in order: 17 rows for CQ-01, 69 for CQ-02,
83 for CQ-03, 71 for CQ-04. Cite at least one reading block per row and per
question. Write each reason in your own words. Copy no source passage into the
record and add no numerical aggregate.

## Recording

Copy `paper-v4/evaluation-v4/run-04/review-record.blank.md`, set `status` to `PRELIMINARY_COMPLETE`, bind the
