# Malleus paper v4 run-21 source-grounded review task

Template, version 4. Instantiate it at freeze by substituting `run-21`,
`24`, `122`, `178`, `166`,
`490` and `182` with the frozen cell's own figures, and
write the result to `paper-v4/evaluation-v4/run-21/review-task.md`. Run-05's
...
`paper-v4/evaluation-v4/run-21/review-input-manifest.json`, which also binds every
...
- `paper-v4/experiment-v4/run-21/results/native-query-binding.json`, the type-only
  binding, expanded from the evaluator's type sets at ontology acceptance and
  before phase two existed.
- `private/paper-v4-v4-run-21/query/query-result.json`, the rows.
- `paper-v4/experiment-v4/run-21/results/trace-summary.json`, provenance for every
  populated record, and
  `paper-v4/experiment-v4/run-21/results/query-trace-summary.json`, the same for
  the 182 witnesses the returned rows use.
- `private/paper-v4-v4-run-21/ledger/retained-capture.json`, the retained capture.
- this task, the frozen protocol, the input manifest, and a copy of
  `paper-v4/evaluation-v4/run-21/review-record.run-21.blank.md`.
...
Judge every returned row exactly once, in order: 24 rows for CQ-01,
122 for CQ-02, 178 for CQ-03, 166 for CQ-04,
490 in all. Cite at least one reading block per row and per question.
Write each reason in your own words. Copy no source passage into the record and
add no numerical aggregate.

## Recording

Copy `paper-v4/evaluation-v4/run-21/review-record.run-21.blank.md`, set `status` to
