...
- `ontology_sha256`: exactly `sha256:7c07f94630277edf4aa1be2515e7627e5ebe42c4c9cfddd6c50b867e9c6291ed`
...
- `ObservationMethod`: `name`, `observation_method_kind`
- `Instrument`: `name`, `instrument_kind`
- `GeologicFeature`: `name`, `geologic_feature_kind`
- `SeismicPhenomenon`: `name`, `seismic_phenomenon_kind`
- `QuantitativeObservation`: `name`, `quantity_kind`, `lower_numeric_value`, `upper_numeric_value`, `measurement_unit`, `observation_basis`
- `GeologicMaterial`: `name`, `geologic_material_kind`
- `ChemicalConstituent`: `name`, `chemical_formula`
- `GeologicProcess`: `name`, `geologic_process_kind`
- `CategoricalObservation`: `name`, `categorical_observation_kind`, `categorical_observation_value`, `observation_basis`
...
- `MethodUsesInstrumentRelation`: source `ObservationMethod`, target `Instrument`
- `FeaturePartOfRelation`: source `GeologicFeature`, target `GeologicFeature`
- `SeismicPhenomenonOccursAtRelation`: source `SeismicPhenomenon`, target `GeologicFeature`
- `ObservationCharacterizesRelation`: source `QuantitativeObservation` or `CategoricalObservation`, target any constructible entity
- `ObservationConcernsConstituentRelation`: source `QuantitativeObservation` or `CategoricalObservation`, target `ChemicalConstituent`
- `MaterialOccursAtRelation`: source `GeologicMaterial`, target `GeologicFeature`
- `ProcessActsOnMaterialRelation`: source `GeologicProcess`, target `GeologicMaterial`
- `ProcessReleasesConstituentRelation`: source `GeologicProcess`, target `ChemicalConstituent`
- `ProcessCausesProcessRelation`: source `GeologicProcess`, target `GeologicProcess`
- `ProcessTriggersSeismicPhenomenonRelation`: source `GeologicProcess`, target `SeismicPhenomenon`
