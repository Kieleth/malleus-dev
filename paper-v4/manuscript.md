...
### 4.1 Calibration: the transcribed Small Shop fixture

Before any model output enters, the same public path is run end to end on the Small Shop fixture. The domain is not ours. It is a controlled transcription of Table 1 of Dirk Fahland's chapter on process mining over multiple behavioral dimensions with event knowledge graphs ([Fahland, 2022](https://doi.org/10.1007/978-3-031-08848-3_9)), chosen because that literature already publishes a worked multi-object example, with orders, inventory items, invoices, payments, supplier orders and a published correction, rather than a domain we would have designed to suit the mechanism. Every fixture input set declares the same attribution: the chapter's DOI, `Table 1` as the source locator, and `CONTROLLED_FIXTURE_TRANSCRIPTION` as the classification.

The transcription is bounded, and each input set names what it does not take from the source: absolute or calendar time, invoice and payment values, actor identity, demand or supply gaps, actions and effects, event-graph nodes, generic fan-out or cardinality, and any accepted-knowledge or protocol outcome. Three consequences follow and are stated rather than glossed. The fixture holds no Event record, because event-graph nodes are outside the transcription, so this calibration exercises entity and relation population only. The settlement relation between a payment and its two invoices is declared by the fixture and not read from the source; the attribution records that as `FIXTURE_DECLARATION_NOT_SOURCE_NATIVE_RELATION`. The synthetic year and UTC offset in the valid times are fixture-derived temporal provenance, not claims the publication makes.
...
Rerunning the fixture reopens the same history and emits byte-identical evidence without changing the ledger. That reproduction holds at the commit carrying this manuscript revision. Four files under the shipped `malleus` package changed between the coordinate the document run is pinned to and this commit (the document adapter's aggregated locator diagnostics, the admission path's handling of list-valued properties, the population compiler, and the pack-grounding rite), and no file under the fixture did; the calibration and the document run therefore share the fixture and the protocol, not one byte-identical implementation.

The fixture is a conformance case, not a protocol rule. The plans are adopter-authored and committed with the fixture under `public_population/plans/`; their format is the fixture's own, not a protocol rule. The state-version history profile is a fixture choice. Event population, semantic re-entry, external effects and a stable wire format are out of its scope. It is evidence about this mechanism only, and nothing here evaluates the chapter it transcribes, its method, or its results.
...
**Text-layer damage is a typed gap, not a repair.** The reading is the PDF text layer as pypdf 6.16.2 extracts it. Where the PDF sets characters individually, the extractor emits a space between letters, so a name arrives as `S a t i s h C . S i n g h`. Twenty-seven of the 186 blocks carry a run like that, almost all in the byline, the acknowledgements and the reference list, and the abstract carries one inside a sentence the producer captured verbatim, spacing included. A producer bound to verbatim capture cannot record a damaged name as a name without reconstructing it, and reconstruction is invention, so the protocol records the field as a `REQUIRED_FIELD_ABSENT_IN_SOURCE` gap and moves on. The remedy is upstream, in the extraction that produces the reading, and re-extracting would change the digest every cell, manifest and review is pinned to; for this paper the damage stays declared, confined to bylines and references, and counted.
...
14. Dirk Fahland. [“Process Mining over Multiple Behavioral Dimensions with Event Knowledge Graphs.”](https://doi.org/10.1007/978-3-031-08848-3_9) In *Process Mining Handbook*, Lecture Notes in Business Information Processing 448, chapter 9. Springer International Publishing (2022).
15. Yohei Nakajima. [“The Log is the Agent: Event-Sourced Reactive Graphs for Auditable, Forkable Agentic Systems.”](https://arxiv.org/abs/2605.21997) arXiv:2605.21997 (2026).
...
The fixture is `OKG-FX001`, Small Shop Fulfilment. Every input set under `research/ontology_driven_kg_realization/fixtures/small_shop_fulfilment*/` declares the same source attribution: DOI 10.1007/978-3-031-08848-3_9, source locator `Table 1`, classification `CONTROLLED_FIXTURE_TRANSCRIPTION`, with the accepted and excluded claims listed per input set in its `attribution.json`.
