...
The recipe library is construction vocabulary, not an answer key. Eight templates encode named entities, observing systems, bounded quantities, mechanism hypotheses, and the ontology's four relation classes. They contain ontology-fixed types and enum values, but no document name, answer value, fixed record identifier, graph size, or causal chain. Recipe expansion produces an ordered plan, and a small adapter converts that plan into the change set used by admission.

The change set is the enforced intermediate representation between generated population and accepted graph state. It binds the selected ontology, source evidence, prior ledger and graph coordinates, operation order, dependencies, and content identity. A separate canonical provenance map associates each proposed record and property with a reading locator and recipe emission; its digest enters the change set's evidence closure. Locators are not inline change-set fields. Missing required data is a refusal. The runner does not invent a locator, type, endpoint, time, base state, or answer.
