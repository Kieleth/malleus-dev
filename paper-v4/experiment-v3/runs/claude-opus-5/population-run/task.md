...
- `ontology_sha256`: exactly `sha256:b0f9d04edbf96df82f97de9d357dd4dea8921fbe96b3f59fcc0c95bd76e90c22`
...
- `SeismicStation`: `latitude`, `longitude`, `station_data_state`
- `SeismicDeployment`: `station_count`, `mean_station_spacing`, `recording_duration`
- `SeismicityDepthSummary`: `maximum_hypocentral_depth`, `depth_datum`, `supporting_event_count`
- `SpreadingSegment`: `axial_length`, `trend_azimuth`, `full_spreading_rate`, `accretion_mode`
- `Earthquake`: `latitude`, `longitude`, `hypocentral_depth`, `depth_datum`, `depth_uncertainty`, `epicentral_uncertainty`, `azimuthal_station_gap`, `travel_time_residual`, `satisfied_criteria_count`
- `ChemicalConcentration`: `analyte_symbol`, `analyte_kind`, `concentration_value`, `concentration_unit`, `determination_mode`
- `MeltPhase`: `melt_stage`
- `ElementRatioProxy`: `numerator_symbol`, `denominator_symbol`, `ratio_value`, `ratio_uncertainty`
- `VolatileSaturationCondition`: `analyte_symbol`, `pressure`, `temperature`
...
- `DeployedInRelation`: source `SeismicStation`, target `SeismicDeployment`
- `SummarizesSeismicityDepthRelation`: source `SeismicityDepthSummary`, target `SpreadingSegment`
- `HostsSeismicityRelation`: source `SpreadingSegment`, target `Earthquake`
- `CharacterizesMeltRelation`: source `ChemicalConcentration`, target `MeltPhase`
- `EstimatedFromRatioRelation`: source `ChemicalConcentration`, target `ElementRatioProxy`
- `SaturatesAtRelation`: source `MeltPhase`, target `VolatileSaturationCondition`
- `GeneratedBeneathRelation`: source `MeltPhase`, target `SpreadingSegment`
