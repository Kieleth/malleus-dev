...
A fresh Claude session performs the preliminary inspection first and records its kind as `CLAUDE_PRELIMINARY`. This is AI-assisted preliminary work, not human evidence. Luis then checks the exact rows and cited blocks, edits the question entries if needed, and records `RATIFIED_AS_RECORDED`, `RATIFIED_WITH_EDITS`, or `REJECTED`. Only a record with top-level status `HUMAN_RATIFIED` is evidence for the paper.
