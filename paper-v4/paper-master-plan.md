...
5. **Population, inputs frozen and acquisition pending.** A different fresh session receives only the corrected ontology, selected reading, ontology-specific generic recipes, four questions, and the closed task. It must use opaque sequential record ids and may return a terminal model refusal. Permit one structural retry, no content repair, and no fallback.
