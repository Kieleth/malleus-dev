...
Status: active. D5 isolation and D1 text-layer reading are complete. The fresh ontology run is next. No document graph or query result is claimed yet.
...
3. Selected ontology digest: pending the fresh ontology run.
...
3. **Ontology, next.** A fresh session receives the selected reading, the four questions, the supported ontology task, and retained copies of every model-visible Malleus input. It returns one ontology file. The compiler may return exact structural diagnostics to that same session at most twice. No human edits the ontology. Once it compiles, one evaluator event records the ontology digest and actor id for population. There is no adequacy review and no `SELECTED` or `REFUSED_ADEQUACY` state.
4. **Query binding.** Before population, bind the four queries to ontology record types, relation types, enum values, joins, and output fields. The binding must not name document answers, numeric values, source text, locators, entity counts, relation counts, or an exact graph closure.
...
1. Copy the exact model-visible inputs once and run D2 with D6.
2. Record the evaluator acceptance event for the compiled ontology.
3. Freeze the type-based queries.
4. Audit the recipe library for genericity, then run D3.
5. Compile, decide, admit, delete, replay, query, and score.
6. Fill result fields, cut the manuscript to target, and build the arXiv bundle.
