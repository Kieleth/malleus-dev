...
| AT-008c | Single-ledger knowledge change | Empty accepted graph plus retained genesis change set; one atomic multi-operation change set; proposal, checks, review, ACCEPT and application sharing one identity; full and incremental replay | External nonempty accepted base, missing or stale base ledger/state coordinate, operation reordering, cross-change dependency, mismatched lifecycle identity, direct accepted-state writer, partial operation application | Exact `KnowledgeChangeSet` identity; ledger-only reconstruction; same full and incremental temporal KG; atomic refusal leaves accepted state unchanged; structural candidates remain non-governed and non-accepted |
