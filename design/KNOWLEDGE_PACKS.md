...
12. Population (after the coverage RCA, `handover/2026-09-03-core-population-adapter-tdd.md`): the unit of capture is the assertion, a verbatim clause with block, modality and attribution, and typed records are its formalizations; the objective is coverage of the reading measured by a per-block census (asserted, unformalised, nothing assertable, untouched), never "smallest"; competency questions enter only the evaluation loop after population is frozen; the constructible set is every concrete type of the accepted ontology; ratification samples blocks. Built by Core, test first, as R1's reference adapter (`handover/2026-09-03-core-population-adapter-tdd.md`).
