...
From the overseer session. Decided by Luis in chat on 2026-09-03. Status 2026-09-03 (late): Core reviewed this and pushed back on eight points; the overseer accepted six, amended one, and pushed back on two. A v2 in Core's own Pareto order is pending two decisions of Luis's (packs as an offered layer; `source-assertion` shipped now with Recon as consumer). Status 2026-09-03 (later): the two decisions are taken and `handover/2026-09-03-core-population-v2.md` is the live Core instruction. It replaces R1 with four ordered pieces (neutral population plan, governed integration, public facade and CLI, document-assertion adapter) and reorders R4, R2, R3, R5, R6 after them as P5 to P9. This file remains the requirement list those pieces serve; where the two differ, v2 wins. Standing order restated: Core touches Core. Core receives requirements with reproducers, reports coordinates and tests, and does not read the paper, its questions, its schemas or its evaluation. The paper thread is an executor and files requirements only through the overseer. The overseer verifies each deliverable on disk before the paper thread is allowed to consume it.
