...
### Stepping back (16:20Z)

No ruling has come; nothing opened. `handover/2026-09-06-stepping-back.md`: five places the complexity sits, with counts from the tree (twenty harness copies, one per cell, six to ten scripts each; the frozen root that fired E-0196's overseer error; the sed-derived freeze and its two defects), the per-question set that never executed (E-0198, E-0199) and the convergence of questions on a coarse surface (run-21's CQ-04 is CQ-03 minus 12 rows), the producer's paraphrase of the adapter, the unwritten responsiveness threshold (kappa 0.118), and the overseer's hand-run scripts. One recommendation, mechanical and no ruling: the harness once at the root with the run id as an argument, pinned per cell like Core. Delivered in chat.

## Luis's turn (2026-09-06, about 17:45Z): two threads reopened

Luis: run a producer on the Small Shop rows the same as for the PDF, by subagent; and keep working on the responsiveness label, an RCA on the grading itself, with the thought of thirty questions of increasing complexity including controls. Paper-26 (Opus) dispatched to build `shop-01` (the four candidate shop questions stated as the overseer's assumption; state-version profile as the fixture's own; the producer writes its own ontology and neutral population plans with row locators; the runner is the public-population admission path). Grading RCA written from the fourteen PARTIAL rationales read in full: one label carries five causes (withheld statement 3, not modelled 4, unreached record 1, superset ambiguity 4, unlinked rows 1, mixed 1), the question file's `required_semantics` is never bound to the review, there is no control question, and four questions converge on a coarse surface. Proposal in `handover/2026-09-06-grading-rca.md`: coverage per required semantic with four typed ABSENT reasons and a derived label, assembly as a descriptor, support once per witness, and a thirty-question set in five tiers with five controls. Delivered in chat; authoring the questions waits on Luis.

## shop-01 (the same skill and Core given rows), opened 2026-09-06T19:27Z at c95dba7 (E-0201, E-0202)

Change under test: the source shape, five Small Shop row files instead of the PDF reading; same skill, same Core coordinate, same producer block (Opus 5). Paper-26 built the cell (8b62122 RED, 919bf8f GREEN; 1,581 active tests) and smoke-ran the runner on the fixture's own plans: the fixture's graph reproduced, the four questions bound and executed (5, 1, 3, 9 rows), and CQ-S2's history half is unreachable by a type-only case, recorded before the producer runs. Two things escalated: the frozen v2 review validator refuses a row-shaped evidence surface (v3 protocol or renamed questions, Luis's), and Core never resolves a derivation locator against its row (a Core gap; reported, not written). The overseer's pin re-run rewrote nothing; E-0202. Producer workspace prepared; launch log opened at paper commit 0f013ba. Expected (E-0201): admission within two structural returns, no UNSUPPORTED row; falsifier a third refusal or an UNSUPPORTED row.

### shop-01, gate (19:42Z)

Opus 5 accepted at attempt 01, 945 facts, seven project classes, every one a direct root extension grounded to schema.org or PROV-O with all ten borrowed terms confirmed by fetch and no none_found block; no pack imported, each declined with a reason. The producer read the state-version profile's empty event role as the load-bearing input and declared no Event class; it carries the warehouse occurrence, the two supplier-order quantities (three readings, no stated order) and the absence of money as typed gaps rather than repairs, and announced six entities and two relations before phase two. Type sets: the types each shop question names, CQ-S4 every project entity type since the question names none; closure clean; 56 cases (11 ENTITY, 45 RELATION, no SUBJECT case: no subject-bearing type on the surface).

Contract identity computed between gate and phase two (validated fact set equal to the gate's). Phase two dispatched 19:45Z (transcript).

### shop-01, population and first runner attempt (19:49Z to 19:58Z)

Opus 5 wrote four plans through a producer-written row adapter: 7 entities, 2 relations, 15 derivations each carrying the source token beside its value, 13 typed gaps, 0 supersessions; every one of the nine physical rows populated or gapped. It declined to invent two coordinates: the plan grammar string and a valid-time kind. Runner attempt 1 refused before the compiler ran: the plan loader requires strict canonical bytes and a grammar key, two rules the skill states only for the document capture and no message stated. Returned as the parent's omitted coordinates, not counted against the two structural returns, with the diagnostic verbatim; recorded in the launch log as HARNESS_INTERFACE_OMISSION_RETURNED_WITH_COORDINATES. Skill gap noted for Core: the structured-source path names no plan grammar and no byte encoding.

### shop-01, runner attempt 2 (19:57Z): the compiler's first word, return 1 of 2

The canonical plans reached the plan compiler, which refused the root closure (FIELDS_NOT_CLOSED): three bookkeeping keys at the root and, below it, an adapter block, source references, derivations carrying the source token beside the value, and a valid_time with no kind. Structural return 1 of 2, the diagnostic verbatim plus the grammar's closed shapes read from the compiler's own field sets, because no declared input states them for structured sources and the fixture plans that show the shape are a forbidden input. Skill gap for Core, second half: the structured-source path names no plan grammar, no root keys, no reference shapes, no valid-time kinds. The producer's checkable `source_value` beside each derivation has no place in the grammar; whether it should is a Core question worth one line.

### shop-01, runner attempt 3 (20:04Z): the parent's wrong digest

The grammar-closed plans reached the compiler and were refused on IDENTITY_MISMATCH: the plans' history_profile digest is the raw digest of the staged profile file, which is what my phase-two message told the producer to compute; the compiler wants the profile's identity, the digest of its canonical bytes, and the staged file is pretty-printed. Corrected coordinate returned, uncounted; one structural return remains. The producer's own report on the rewrite named an expressiveness gap in the grammar: valid_time is required and no kind can say a change set has no valid time; it used ORDER_ONLY "0" in all four plans and said so. Three refusals so far, two of them the parent's: the shape of the structured path is what the document cells never exercised.

### shop-01, admitted and queried (20:08Z)

Admitted at runner attempt 4 with the profile identity corrected: 7 entities, 2 relations, 15 derivations, 13 typed gaps (11 TYPE_ABSENT, 2 RELATION_ABSENT), 0 supersessions, 38 ledger events, replay reproducing admission, 9 records traced. No census: the plan compiler reports none for rows, as the contract said. The binding frozen at acceptance executed unchanged after replay: 17 rows (5, 1, 2, 9; 13 ENTITY, 4 RELATION) over 9 witnesses, no forbidden attempt; CQ-S2's one row is the current state with no history, as recorded before the producer ran. Producer 250,743 tokens, 138,864 of them on the ontology. The four hand-offs changed no modelling decision; three refusals changed the plan's shape and one digest rule.

### shop-01 freeze, first pass (20:45Z): three wordings caught, one red gate of mine

Paper-27 froze 27 public artifacts (nothing withheld: the five sources are the repository's fixtures), wrote the review manifest unvalidated with the reason, and stopped at the ledger: the E-0203 body I supplied said "twelve rows" where the files say nine (twelve is the declared-input count), "three of the four refusals" where there are three, and "7 entity types beside the roots" where six sit beside the root. All three corrected in the body before the append; the rule that no count enters a record before the script that shows it was broken by me on the first. A second error of mine: the E-0202 commit (0f013ba) appended to the ledger after the gate had run, and the E-0201 guard split the ledger at its heading and took the rest of the file, so the suite was red (1 failed) from that commit until Paper-27 bounded the guard. Rule restated: the gate runs after every ledger append, not before.

### shop-01 closed (21:00Z): the review found the convention nobody declared

Two sessions, 17 rows: 2 SUPPORTED, 5 PARTIAL, 10 UNSUPPORTED; every question PARTIAL; E-0204. The falsifier fired on E-0201's second half and the cause is not the producer: it numbers rows from 1 by physical line and said so; the review task numbers from 0 by data row, from the fixture; nothing declared either, and Core resolves no locator. Under the producer's convention all 15 derivations resolve and 9 values are equal, the other 6 being a constant and four prefixed identifiers that the task's equality token fails and the grammar's derivation rule allows; the fixture's own plan has the same shape. The two sessions labelled the same unresolvable locator PARTIAL and UNSUPPORTED. RCA at `handover/2026-09-06-shop-01-rca.md`: seven causes, two of them the parent's coordinates, two the protocol's silences (row numbering, derivation versus equality), one the harness's staging, one the grammar's (no valid time), one Core's (locators unresolved), and the review protocol unbound. The merge script's locator check flagged every locator because the reviewers named the file first as the task told them; the record is complete and the script's pattern was wrong. Rulings added for Luis: the label for an unresolvable locator, whose convention the cell is under, whether to re-review.

## Luis's rulings (2026-09-06, about 20:55Z): the loop resumes on seven decisions

Verbatim in E-0205. In flight since 20:55Z, in parallel on disjoint files: a fresh Opus session authoring thirty questions from the reading alone (paper-v4/experiment-v4/competency-questions-v3.json, draft for freeze); Core-21 (Opus) re-freezing the Small Shop public evidence at the current Core and writing the skill's structured-source section with a compiler-validated worked plan, one writer on Core's ledger; Paper-28 (Opus) writing review protocol v3 and its validator (declared surface and locator convention, coverage per semantic with a derived label, assembly descriptor, controls, NOT_EVALUABLE by rule for an unresolvable locator). Eleven human records written and validated for runs 09 to 21; run-20 the cell of record. After these land: Core-22 (locator resolution at admission with a declared convention) and Core-23 (a no-valid-time kind) in sequence on Core's ledger; harness v4.12 (reachability (b), canonical profile staging); then replicates of the record condition, three to five, reviewed under v3. Agents do not write the paper ledger; the overseer does.
...
## Run-22 (v4.12, Opus 5, second replicate of the record condition under protocol v3 and the thirty questions), opened 2026-09-06T22:45Z at c95dba7 (E-0209, E-0210)

Paper-29 built harness v4.12 (765862e, db6cb22; 1,746 active tests): ENTITY_NO_SUBJECT reach (binding schema v5), the closure check inside the binder behind a required --contract, the profile staged canonical; the review package instantiated from protocol v3 for the thirty question ids; the measured_against block recomputed from run-21's frozen files rather than carried under a new name. On run-09's frozen record the new case kind restores 429 rows over 373 witnesses, 427 SUPPORTED. The overseer's pin re-run rewrote nothing; E-0210. Expected: admission within two structural returns, no UNSUPPORTED witness. Comparable with run-20 and run-21 on the producer side only.

Run-22's producer (Opus 5) launched at 22:55Z on the spawn message with the run id moved (diffed against run-21's: the id only), workspace staged at db6cb22 with the profile canonical, launch log's paper commit set to 59c3159 (the E-0209/E-0210 commit) after an ordering slip: the workspace and the log were prepared before the entries' gate had passed, and the first gate failed on two phrases the cell's opening-entry test requires ("Claude Opus 5" and run-21's 490 rows), added before the commit. Core-22's report still pending; its GREEN (1324ccc) is on main.
...
## Governed loop, night of 2026-09-10 to 11: Sonnet run 06 ends on the cap; the comparator repaired after Codex's audit

Run 06 (Sonnet, v2, corroboration, replay cache) ran 40 batches and ended on the cap with the checker at the button stage: never ahead of the checker, 3 behind at the first placement and 1 at the second, four holds, button instruction from batch 33 against 27 in run 02; the completed runs needed 5 to 10 batches of that instruction, so the cap decided it. 40 admitted, 0 refusals, 5 h 17 min with the cache. Codex's robotics thread returned early and audited the comparator, correctly: producer_version defaulted, equality on id and count only. Repaired on the branch with seven counterexample tests; runs 01, 02, 03 and 06 tie exactly under the repaired tool. Branch commits 42d84f0c, c92ba5c2; ledger E-0354; gate 20.

## Fault injection through three stores (2026-09-11): same refusals as the event log on proposals, provenance binding only on the Malleus arm

Luis ruled "1 and then 2": faults first, then the larger-cap Sonnet rerun. Fourteen faults at five turns of run 03's history through a Malleus prefix arm, the event log and the naive store. Malleus and the event log agree on all eleven proposal-level faults; the naive store admits four that change its memory; all three admit a plausible wrong count (the evidence-rule target); only Malleus refuses fabricated captures, the other two admit them and cite them. The first pass showed the Malleus adapter journaled only what reached Core; stores.py now journals every pre-Core decision on both arms, suite 796. Branch b8757687, 9fae9da1; ledger E-0355; gate 21. Run 07 (Sonnet, 48 batches) launched after the first pass was committed and is being dispatched turn by turn.

## The evidence rule at admission (2026-09-11): one stage per assessment, declared span, shared by both stores

Second increment of the pair recorded in E-0354. Rule version 2 in the shared revision check; adapters declare the span. Replayed on every retained run: Opus and Sonnet untouched, Haiku run 04 refused at every turn as a jump (the runaway stops at admission, no pixels), run 01's one-stage false advance inside the rule. Fault harness pass 3 under the rule: plus-three and jump-of-two refused as jumps, no-span advance refused, by both stores. Branch 7da827b2; ledger E-0356; gate 22. Run 07 (Sonnet, 48 batches) still running under rule-1 code, at the first placement.
...
## M3 (2026-09-12 early): L1 completes from an empty graph; the transfer runs start

Sonnet as operator, no hand-written cue, 48-batch cap: success at batch 36, elapsed 571, 64 findings, 0 refusals, 1785 events, 8 h. Pickups in the checker's batch, placements one after (definitional). Comparator ties. Recorded: near-duplicate heuristics for A5; a disclosed metadata read at turn 32; the clone launch failure from a relative path inside the archive cwd, launcher fixed. Runs 10 (clone, seed 1000) and 11 (empty, seed 1011) launched in parallel, then stopped at batches 2 and 3: their ids carried the action chunk's text (the loop bound the chunk to the name prefix); set aside, guard and test added, relaunched. Ledger E-0361; gate 27 failed on the Codex paper thread's live edits under answer-demonstration, gate rerun on a clean worktree of HEAD plus these two files.
